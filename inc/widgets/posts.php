...
function the_business_wp_latest_posts_widget() {
	register_widget( 'the_business_wp_latest_posts_widget' );
}
add_action( 'widgets_init', 'the_business_wp_latest_posts_widget' );

// Creating the widget 
class the_business_wp_latest_posts_widget extends WP_Widget {

function __construct() {
parent::__construct(

// Base ID of your widget
'the_business_wp_latest_posts_widget', 

// Widget name will appear in UI
__('Advanced Recent Posts', 'the-business-wp'), 

// Widget description
array( 'description' => __( 'Display latest_posts with featured image and Meta', 'the-business-wp' ), ) 
);
}
...
$title = ( ! empty( $instance['title'] ) ) ? strip_tags( apply_filters( 'widget_title', $instance['title'] ) ) : __( 'Recent Posts', 'the-business-wp' );
...
the_business_wp_get_latest_posts($max_items);
...
<p>
<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:','the-business-wp' ); ?></label> 
<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
</p>
<p>
<label for="<?php echo esc_attr($this->get_field_id( 'max_items' )); ?>"><?php esc_html_e( 'Number of posts to Show:','the-business-wp'  ); ?></label> 
...
<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php esc_html_e( 'Hide Widget Title','the-business-wp' ); ?></label> 
</p>
<?php 
}
...
public function update( $new_instance, $old_instance ) {
	$instance = array();
	$instance['title'] = ( ! empty( $new_instance['title'] ) ) ? sanitize_text_field( $new_instance['title'] ) : esc_html_e( 'Advanced Recent Posts','the-business-wp' );
...
function the_business_wp_get_latest_posts($max){
